...
resource "aws_instance" "simple_ec2_instance" {
  for_each = { for name in var.environment_suffix : name => format("${name}-%s", "${name}") }
  ami                    = data.aws_ami.special_ami.id
  instance_type          = var.ec2_config["instance_type"] #var.ec2_config.instance_type
  key_name               = data.aws_key_pair.aprilkey.key_name
  tags = {
    Name = "Simple_ec2_with_for_loop-${each.value}"
  }
}
